...
import { UpdateTableBody, UpdateTableBodyType } from '@/zusstand/table/table.schema'
import { useGetTableQuery, useUpdateTableMutation } from '@/zusstand/table/table-client-controler'
...
export default function EditTable({
  ...
  const { data } = useGetTableQuery({  id: id })
  ...
  const onSubmit = async (values: UpdateTableBodyType) => {
    ...
    try {
      if (!data) throw new Error("Table data not available")
      let body: UpdateTableBodyType & { number: number } = {
        number: data.number,
        ...values
      }
      const result = await updateTableMutation.mutateAsync(body)
      toast({
        description: "success full upload"
      })
      reset()
