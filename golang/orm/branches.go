...
type branchR struct {
	Manager     *Account        `boil:"Manager" json:"Manager" toml:"Manager" yaml:"Manager"`
...
func (o *Branch) GetManager() *Account {
	if o == nil {
		return nil
	}
...
func (r *branchR) GetManager() *Account {
	if r == nil {
		return nil
	}
...
func (o *Branch) Manager(mods ...qm.QueryMod) accountQuery {
...
func (branchL) LoadManager(ctx context.Context, e boil.ContextExecutor, singular bool, maybeBranch interface{}, mods queries.Applicator) error {
	var slice []*Branch
	var object *Branch
	...
	if singular {
		...
		foreign.R.ManagerBranches = append(foreign.R.ManagerBranches, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if queries.Equal(local.ManagerID, foreign.ID) {
				...
				foreign.R.ManagerBranches = append(foreign.R.ManagerBranches, local)
				break
			}
		}
	}
...
// SetManager of the branch to the related item.
// Sets o.R.Manager to related.
// Adds o to related.R.ManagerBranches.
func (o *Branch) SetManager(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Account) error {
	var err error
	if insert {
	...
	if related.R == nil {
		related.R = &accountR{
			ManagerBranches: BranchSlice{o},
		}
	} else {
		related.R.ManagerBranches = append(related.R.ManagerBranches, o)
	}
...
func (o *Branch) RemoveManager(ctx context.Context, exec boil.ContextExecutor, related *Account) error {
	var err error
	...
	for i, ri := range related.R.ManagerBranches {
		if queries.Equal(o.ManagerID, ri.ManagerID) {
			continue
		}

		ln := len(related.R.ManagerBranches)
		if ln > 1 && i < ln-1 {
			related.R.ManagerBranches[i] = related.R.ManagerBranches[ln-1]
		}
		related.R.ManagerBranches = related.R.ManagerBranches[:ln-1]
		break
	}
	return nil
}
