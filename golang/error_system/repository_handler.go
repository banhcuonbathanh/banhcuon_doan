...
// 	// Handle database-specific errors with enhanced logging
// 	appErr := h.handleDatabaseError(err, operation, table, context)
// 	h.logError(appErr, operation, table, context)
	
// 	return appErr
// }

// func (h *RepositoryErrorHandler) handleDatabaseError(err error, operation, table string, context map[string]interface{}) *AppError {
// 	errStr := strings.ToLower(err.Error())
	
// 	// Enhanced logging context with operation details
// 	enhancedContext := make(map[string]interface{})
// 	for k, v := range context {
// 		enhancedContext[k] = v
// 	}
// 	enhancedContext["original_error"] = err.Error()
// 	enhancedContext["error_analysis"] = "database_error_categorization"
	
// 	// Check for duplicate key violations - MOST SPECIFIC FIRST
// 	if strings.Contains(errStr, "duplicate") || strings.Contains(errStr, "unique constraint") {
// 		if strings.Contains(errStr, "email") {
// 			// Log specific error cause for better debugging
// 			h.logErrorWithCause(core.CauseDuplicateEmail, operation, table, enhancedContext)
// 			return NewErrorWithInternalAndMessages(
// 				ErrAccountDuplicate,
// 				"Account with this email already exists",
// 				"Tài khoản với email này đã tồn tại",
...
// 			)
// 		}
// 		h.logErrorWithCause("duplicate_entry", operation, table, enhancedContext)
// 		return NewErrorWithInternalAndMessages(
// 			ErrValidationFailed,
// 			"Duplicate entry detected",
// 			"Phát hiện bản ghi trùng lặp",
...
// 	// Check for foreign key constraint violations
// 	if strings.Contains(errStr, "foreign key") || strings.Contains(errStr, "fk_") {
// 		h.logErrorWithCause("constraint_violation", operation, table, enhancedContext)
// 		return NewErrorWithInternalAndMessages(
// 			ErrValidationFailed,
// 			"Referenced record does not exist",
// 			"Bản ghi được tham chiếu không tồn tại",
...
// 	if strings.Contains(errStr, "connection") || strings.Contains(errStr, "network") || 
// 	   strings.Contains(errStr, "connect") || strings.Contains(errStr, "refused") {
// 		h.logErrorWithCause(core.CauseNetworkError, operation, table, enhancedContext)
// 		return NewErrorWithInternalAndMessages(
// 			ErrServiceUnavailable,
// 			"Database connection error",
// 			"Lỗi kết nối cơ sở dữ liệu",
...
// 	// Check for timeout errors
// 	if strings.Contains(errStr, "timeout") || strings.Contains(errStr, "deadline exceeded") {
// 		h.logErrorWithCause(core.CauseTimeout, operation, table, enhancedContext)
// 		return NewErrorWithInternalAndMessages(
// 			ErrTimeout,
// 			"Database operation timeout",
// 			"Thao tác cơ sở dữ liệu hết thời gian chờ",
...
// 	// Check for transaction errors
// 	if strings.Contains(errStr, "transaction") || strings.Contains(errStr, "rollback") {
// 		h.logErrorWithCause(core.CauseDatabaseError, operation, table, enhancedContext)
// 		return NewErrorWithInternalAndMessages(
// 			ErrDatabaseError,
// 			"Transaction failed",
// 			"Giao dịch thất bại",
...
// 	if strings.Contains(errStr, "permission") || strings.Contains(errStr, "access denied") ||
// 	   strings.Contains(errStr, "insufficient") {
// 		h.logErrorWithCause("access_denied", operation, table, enhancedContext)
// 		return NewErrorWithInternalAndMessages(
// 			ErrForbidden,
// 			"Database access denied",
// 			"Truy cập cơ sở dữ liệu bị từ chối",
...
// 	if strings.Contains(errStr, "table") && (strings.Contains(errStr, "not found") || strings.Contains(errStr, "doesn't exist")) ||
// 	   strings.Contains(errStr, "column") && (strings.Contains(errStr, "not found") || strings.Contains(errStr, "doesn't exist")) {
// 		h.logErrorWithCause("schema_error", operation, table, enhancedContext)
// 		return NewErrorWithInternalAndMessages(
// 			ErrSystemError,
// 			"Database schema error",
// 			"Lỗi cấu trúc cơ sở dữ liệu",
...
// 	// Check for data too long errors
// 	if strings.Contains(errStr, "data too long") || strings.Contains(errStr, "value too long") {
// 		h.logErrorWithCause("data_too_long", operation, table, enhancedContext)
// 		return NewErrorWithInternalAndMessages(
// 			ErrValidationFailed,
// 			"Data exceeds maximum length",
// 			"Dữ liệu vượt quá độ dài tối đa",
...
// 	if strings.Contains(errStr, "disk full") || strings.Contains(errStr, "no space") ||
// 	   strings.Contains(errStr, "storage") {
// 		h.logErrorWithCause("storage_error", operation, table, enhancedContext)
// 		return NewErrorWithInternalAndMessages(
// 			ErrServiceUnavailable,
// 			"Database storage error",
// 			"Lỗi lưu trữ cơ sở dữ liệu",
